...
namespace HermesProxy.World.Client
{
    ...
    public class WlkWorldCrypt : LegacyWorldCrypt
    {
        ...
        public void Initialize(byte[] sessionKey)
        {
            ...
            byte[] ServerDecryptionKey = new byte[16] { 0xCC, 0x98, 0xAE, 0x04, 0xE8, 0x97, 0xEA, 0xCA, 0x12, 0xDD, 0xC0, 0x93, 0x42, 0x91, 0x53, 0x57 };
            byte[] ServerEncryptionKey = new byte[16] { 0xC2, 0xB3, 0x72, 0x3C, 0xC6, 0xAE, 0xD9, 0xB5, 0x34, 0x3C, 0x53, 0xEE, 0x2F, 0x43, 0x67, 0xCE };

            // create RC4-drop[1024] stream
            using (HMACSHA1 outputHMAC = new HMACSHA1(ServerEncryptionKey))
                _serverEncrypt = new ARC4(outputHMAC.ComputeHash(sessionKey));
            _serverEncrypt.Process(new byte[1024], 0, 1024);

            // create RC4-drop[1024] stream
            using (HMACSHA1 inputHMAC = new HMACSHA1(ServerDecryptionKey))
                _clientDecrypt = new ARC4(inputHMAC.ComputeHash(sessionKey));
            _clientDecrypt.Process(new byte[1024], 0, 1024);
        ...
        public void Decrypt(byte[] data, int len)
        {
            _clientDecrypt.Process(data, 0, len);
        }

        public void Encrypt(byte[] data, int len)
        {
            _serverEncrypt.Process(data, 0, len);
        }
